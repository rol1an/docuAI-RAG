...
**docuai** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg)
, JWT + API Key auth, LangChain, RAG (pgvector), Next.js 15 (i18n)
...
uv run docuai rag-ingest /path/to/file.pdf --collection docs
uv run docuai rag-search "query" --collection docs

# Sync Sources
uv run docuai cmd rag-sources
uv run docuai cmd rag-source-add
uv run docuai cmd rag-source-sync
```
