...
```bash
cd backend
uv run docuai <group> <command> [options]
```

### Server Commands

```bash
uv run docuai server run              # Start dev server
uv run docuai server run --reload     # With hot reload
uv run docuai server run --port 9000  # Custom port
uv run docuai server routes           # Show all registered routes
```

### Database Commands

```bash
uv run docuai db init                  # Run all migrations
uv run docuai db migrate -m "message"  # Create new migration
uv run docuai db upgrade               # Apply pending migrations
uv run docuai db upgrade --revision e3f  # Upgrade to specific revision
uv run docuai db downgrade             # Rollback last migration
uv run docuai db downgrade --revision base  # Rollback to start
uv run docuai db current               # Show current revision
uv run docuai db history               # Show migration history
```
...
uv run docuai user create

# Create user non-interactively
uv run docuai user create --email user@example.com --password secret

# Create user with specific role
uv run docuai user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run docuai user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run docuai user create-admin --email admin@example.com --password secret

# Change user role
uv run docuai user set-role user@example.com --role admin

# List all users
uv run docuai user list
```
...
```bash
uv run docuai cmd <command-name> [options]
```
...
uv run docuai cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run docuai cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run docuai cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run docuai cmd rag-ingest ./docs/ --sync-mode new_only
uv run docuai cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run docuai cmd rag-ingest ./docs/ --no-replace
```
...
uv run docuai cmd rag-search "what is fastapi"

# Search a specific collection
uv run docuai cmd rag-search "deployment guide" --collection docs

# Get more results
uv run docuai cmd rag-search "deployment" --top-k 10
```
...
uv run docuai cmd rag-collections

# Show overall RAG system statistics
uv run docuai cmd rag-stats

# Drop a collection (with confirmation)
uv run docuai cmd rag-drop my_collection

# Drop without confirmation
uv run docuai cmd rag-drop my_collection --yes
```
...
uv run docuai cmd rag-sources

# Add a new sync source
uv run docuai cmd rag-source-add \
    --name "My Drive" \
    --type gdrive \
    --collection docs \
...
uv run docuai cmd rag-source-remove <source-id>
uv run docuai cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run docuai cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run docuai cmd rag-source-sync --all
```
...
```bash
uv run docuai cmd my-command --name test
```
