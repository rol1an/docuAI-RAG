...
class RetrievalService(BaseRetrievalService):
    ...
    async def retrieve(
        ...
        # Key includes page_num because chunk_num resets to 0 on each page.
        # Content hash catches identical text from duplicate ingestions (different parent_doc_id).
        ...
        for r in filtered_results:
            ...
            if r.parent_doc_id:
                ...
                chunk_num = r.metadata.get("chunk_num", "")
                key = f"{r.parent_doc_id}:{page_num}:{chunk_num}:{content_hash}"
            else:
                key = content_hash
    ...
    async def retrieve_multi(
        ...
        # Deduplicate across collections — same logic as retrieve()
        seen_keys: set[str] = set()
