...
quickstart: install docker-up
	...
	cd backend && uv run docuai db migrate -m "initial" || true
	@echo "Applying migrations..."
	cd backend && uv run docuai db upgrade
	@echo "Creating admin user (admin@example.com / admin123)..."
	cd backend && uv run docuai user create --email admin@example.com --password admin123 --superuser 2>/dev/null || true
	@echo ""
	@echo "🚀 Ready! All services running:"
	@echo "   API:      http://localhost:8000"
	@echo "   Docs:     http://localhost:8000/docs"
	@echo "   Frontend: Run 'cd frontend && bun install && bun dev' or 'make docker-frontend'"
...
db-migrate:
	...
	uv run --directory backend docuai db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend docuai db upgrade

db-downgrade:
	uv run --directory backend docuai db downgrade

db-current:
	uv run --directory backend docuai db current

db-history:
	uv run --directory backend docuai db history
...
run-prod:
	uv run --directory backend docuai server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend docuai server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend docuai user create-admin

user-create:
	uv run --directory backend docuai user create

user-list:
	uv run --directory backend docuai user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/docuai"
...
help:
	@echo ""
	@echo "docuai - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Quick Start:"
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run docuai rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run docuai rag-search <query> -c <collection>  Search"
	@echo "  uv run docuai rag-collections                     List collections"
	@echo "  uv run docuai rag-sources                         List sync sources"
	@echo "  uv run docuai rag-source-add                      Add sync source"
	@echo "  uv run docuai rag-source-sync <id>                Trigger sync"
	@echo ""
